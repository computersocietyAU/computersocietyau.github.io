...
function MediumBlog() {
  ...
  return (
    <div className="pt-[7em] px-7 pb-7">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:mx-8 gap-y-7 gap-x-8 h-full">
        {/* <MediumBlogCard/> */}
        {/* {blogs?.map()} */}
        {
          data.blogs.map(blog => (
            <a href={blog.link} target="_blank" rel="noreferrer noopener">
              <MediumBlogCard imageURL={blog.description.toString().match(/<img[^>]+src="([^">]+)"/)[1]} title={blog.title} link={blog.link} body={blog.content} />
            </a>
          ))}
      </div>
    </div>
